...
const Navbar = () => {
  ...
  return (
    <div className=' sticky top-0 left-0 right-0 z-20 bg-white shadow-md'>
      <div className='flex items-center justify-between py-5 px-4 sm:px-8 font-bold'>
        <Link to='/'>
          <img src={assets.logo} className='w-40' alt="brand logo" />
        </Link>

        {/* Shaped UL Navigation */}
        <ul className='hidden sm:flex gap-8 text-sm text-gray-800'>
          <NavLink
            to='/'
            className={({ isActive }) =>
              isActive
                ? 'relative flex items-center justify-center px-4 py-2 bg-yellow-500 text-black text-base rounded-full font-bold shadow-lg'
                : 'relative flex items-center justify-center px-4 py-2 bg-teal-500 text-white text-base rounded-full transition duration-300 ease-in-out transform hover:-translate-y-1 hover:bg-yellow-500 shadow-lg'
            }
          >
            <span>HOME</span>
          </NavLink>
        ...
        <NavLink
          ...
          <div className='relative group'>
            ...
            {token && (
              <div className='group-hover:block hidden absolute right-0 px-4 pt-4'>
                <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-yellow-500 text-white rounded'>
          ...
          <img onClick={() => setVisible(true)} src={assets.menu_icon} className='w-5 cursor-pointer sm:hidden mx-5' alt="menu bar icon" />
        </div>
      </div>

      {/* Sidebar menu for small screens */}
      <div
        className={`fixed top-0 right-0 bottom-0 bg-white transition-all duration-300 overflow-hidden ${visible ? 'w-full' : 'w-0'} z-30`}
      >
